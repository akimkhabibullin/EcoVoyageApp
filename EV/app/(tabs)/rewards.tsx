...
export default function RewardsScreen() {
  ...
  const [points, setPoints] = useState<number>(Number(localStorage.getItem("points")));
      ...
      <Pressable 
        ...
        onPress={() => {
          if (points >= item.pointsRequired) {
            ...
            updateUserInfo(uid, { points: Number(localStorage.getItem("points")) })

            // update achievements
            achievements[item.id] = true;
            let achievementsToSave = JSON.stringify(achievements);
            setAchievements(achievementsToSave);
            localStorage.setItem("achievements", achievementsToSave);
            updateUserInfo(uid, { achievements: achievementsToSave })
          }
          }
        }
      >
  ...
  return (
    <View style={styles.container}>
      <Pressable onPress={() => setPoints(points + 100)}>add 100 points! (testing)</Pressable>
